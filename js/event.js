...
function renderPublicTimeline(date, responses) {
  ...
  return `<div class="public-day-block no-scroll-timeline">
    <div class="public-time-table dense-time-table" style="--cols:${EVENT_SLOTS.length}">
...
function renderAnswerTimeline() {
  const groups = groupedCandidates();
  const html = groups.map(([date]) => `<div class="answer-day-block no-scroll-timeline">
    <div class="answer-time-table dense-time-table" style="--cols:${EVENT_SLOTS.length}">
      <div class="public-date-cell sticky-left">${formatEventDate(date)}</div>
      ${EVENT_SLOTS.map(slot => `<div class="public-hour-head">${slot.endsWith(':00') ? slot.slice(0,2) : ''}</div>`).join('')}
...
function paintAnswerCell(cell) {
  if (!cell || !cell.dataset || !cell.dataset.cid) return;
  const cid = cell.dataset.cid;
  if (lastTouchedCid === cid && isDraggingAnswer) return;
  lastTouchedCid = cid;
  answerMap.set(cid, currentMark);
  document.querySelectorAll(`#answerTimeline .answer-slot[data-cid="${CSS.escape(cid)}"]`).forEach(target => {
    target.className = `answer-slot candidate ${statusClass(currentMark)}`;
    target.innerHTML = `<span>${TimeGridApi.markMap[currentMark]}</span>`;
  });
}
function cellFromPointerEvent(e) {
  const el = document.elementFromPoint(e.clientX, e.clientY);
  return el ? el.closest('#answerTimeline .answer-slot.candidate') : null;
}
function openModal() {
  answerMap = new Map();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('answerTimeline').addEventListener('pointerdown', e => {
    const cell = e.target.closest('.answer-slot.candidate');
    if (!cell) return;
    e.preventDefault();
    isDraggingAnswer = true;
    lastTouchedCid = '';
    paintAnswerCell(cell);
  });
  document.addEventListener('pointermove', e => {
    if (!isDraggingAnswer) return;
    e.preventDefault();
    paintAnswerCell(cellFromPointerEvent(e));
  }, { passive: false });
