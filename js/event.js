...
function slotKey(date, startTime) { return `${date}__${startTime}`; }
...
async function load() {
  ...
  try {
    eventData = await TimeGridApi.post('getEvent', { eventId: id });
...
function renderPublicTimeline(date, responses) {
  ...
  return `<div class="public-day-block">
    <div class="public-time-table" style="--cols:${EVENT_SLOTS.length}">
      <div class="public-date-cell sticky-left">${TimeGridApi.formatDate(date)}</div>
